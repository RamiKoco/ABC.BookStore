...
[Authorize(BookStorePermissions.Banka_.Default)]
public class BankaAppService : BookStoreAppService, IBankaAppService
{
    ...
    public virtual async Task<PagedResultDto<ListBankaDto>> GetListAsync(BankaListParameterDto input)
    {
        var entities = await _bankaRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount,
            ...
            b => b.OzelKod3,
            b => b.OzelKod4,
            b => b.OzelKod5);//include properties
    ...
    [Authorize(BookStorePermissions.Banka_.Create)]
    public virtual async Task<SelectBankaDto> CreateAsync(CreateBankaDto input)
    {
        await _bankaManager.CheckCreateAsync(input.Kod, 
            input.OzelKod1Id, 
            input.OzelKod2Id,
            input.OzelKod3Id,
            input.OzelKod4Id,
    ...
    [Authorize(BookStorePermissions.Banka_.Update)]
    public virtual async Task<SelectBankaDto> UpdateAsync(Guid id, UpdateBankaDto input)
    {
        var entity = await _bankaRepository.GetAsync(id, b => b.Id == id);

        await _bankaManager.CheckUpdateAsync(id, input.Kod, entity, 
            input.OzelKod1Id,
            input.OzelKod2Id,
            input.OzelKod3Id,
            input.OzelKod4Id,
    ...
    [Authorize(BookStorePermissions.Banka_.Delete)]
