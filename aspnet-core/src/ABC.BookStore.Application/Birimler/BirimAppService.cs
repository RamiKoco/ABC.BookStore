...
[Authorize(BookStorePermissions.Birim.Default)]
public class BirimAppService : BookStoreAppService, IBirimAppService
{
    ...
    [Authorize(BookStorePermissions.Birim.Create)]
    public virtual async Task<SelectBirimDto> CreateAsync(CreateBirimDto input)
    {
        await _birimManager.CheckCreateAsync(input.Kod, 
            input.OzelKod1Id, 
            input.OzelKod2Id,
            input.OzelKod3Id,
            input.OzelKod4Id,
    ...
    [Authorize(BookStorePermissions.Birim.Update)]
    ...
    [Authorize(BookStorePermissions.Birim.Delete)]
