...
export default function DepoPage() {
  ...
  const fetchDepolar = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get("/api/app/depo", {
        params: { durum: durumFilter, skipCount: 0, maxResultCount: 5000 },
      });
      setDepolar(response.data.items);
    } catch (error) {
  ...
  const getNewCode = async () => {
    try {
      const response = await apiClient.get("/api/app/depo/code", {
        params: { durum: true },
      });
  ...
  useEffect(() => {
    if (auth.isAuthenticated) fetchDepolar();
  ...
  const handleEdit = async (depo: Depo) => {
    try {
      const response = await apiClient.get(`/api/app/depo/${depo.id}`);
      ...
      console.error("Depo detayı yüklenemedi:", error);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingDepo) {
        await apiClient.put(`/api/app/depo/${editingDepo.id}`, form);
      } else {
        await apiClient.post("/api/app/depo", form);
      }
      setDrawerOpen(false);
      setEditingDepo(null);
      setForm({ ...emptyForm });
  ...
  const handleDelete = async (id: string) => {
    try {
      await apiClient.delete(`/api/app/depo/${id}`);
      fetchDepolar();
    } catch (error: any) {
  ...
  return (
    <>
      <DevListPageLayout
        loading={loading && depolar.length === 0}
        loadingText="Depolar yükleniyor..."
        editVisible={drawerOpen}
        editTitle={editingDepo ? "Depo Düzenle" : "Yeni Depo"}
